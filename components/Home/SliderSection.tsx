...
export default function HeroSlider({ slides }: { slides: Home["slides"] }) {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          transform: `translateX(${
            locale === "ar" ? current * 100 : -current * 100
          }%)`,
        }}
      >
      ...
      {t(slide.title)}
                ...
                <p className="font-bold text-4xl md:text-6xl mb-6">
                  {t(slide.subtitle)}
                </p>
              </div>

              <div
                className={`absolute mt-64 flex items-center w-64 ${
              ...
              >
                ...
                <Link href="/service">
                  <button className="relative w-16 h-16 bg-[#00A699] rounded-full hover:bg-[#008578] transition cursor-pointer">
                    <p
                      className={`absolute  bottom-5 font-normal text-white ${
      ...
      {slides.length > 1 && (
        <>
          <div
            className={`absolute bottom-6  bg-transparent flex items-end gap-2 ${
